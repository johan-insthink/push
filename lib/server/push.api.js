...
Push.Configure = function(options) {
    ...
    if (options.apn) {
        ...
        var apnProdConnection = new apn.Connection( options.apnProd );
        ...
        apnConnection.on('transmissionError', Meteor.bindEnvironment(function (errCode, notification, recipient) {
          ...
          if ([2, 5, 8].indexOf(errCode) >= 0) {
            ...
            if (errCode === 8) {
              console.log("Can't send with dev-settings");
            } else {
        ...
        // Listen to transmission errors - should handle the same way as feedback.
        apnProdConnection.on('transmissionError', Meteor.bindEnvironment(function (errCode, notification, recipient) {
          if (Push.debug) {
            console.log('Prod notification - Got code %d for token %s', errCode, notification.token);
          }
        }));
        ...
        self.sendAPN = function(userToken, notification, useProd) {
          ...
          // adds category support for iOS8 custom actions as described here:
            ...
            if (useProd) {
              apnProdConnection.pushNotification(note, myDevice);
            } else {
            ...
            if (app.token.apn) {
              countApn.push(app._id);
                ...
                if (self.sendAPN) {
                  ...
                  // Also try prod send
                  self.sendAPN(app.token.apn, options, true);
                }
